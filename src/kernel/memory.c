...
static void mem_pool_init(uint32_t all_mem) {
    ...
    uint32_t page_table_size = PG_SIZE * 256;            // 1页目录表+第0和第768个页目录项指向同一个页表+
    // 769~1022页目录项共指向254个页表,共256个页框
    uint32_t used_mem = page_table_size + 0x100000;      // 0x100000为低端1M内存
    uint32_t free_mem = all_mem - used_mem;             // 内存管理系统在1MB内，方便了计算
    uint16_t all_free_pages = free_mem / PG_SIZE;       // 以页为单位的分配,不管free是不是4k的倍数,不用考虑
    uint16_t kernel_free_pages = all_free_pages / 2;    // 目前是内核和用户一人一半
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;
    
    // 为简化位图操作，余数不处理，坏处是这样做会丢内存。
    // 好处是不用做内存的越界检查,因为位图表示的内存少于实际物理内存
    uint32_t kbm_length = kernel_free_pages / 8;         // Kernel BitMap的长度, 一位表示一页,以字节为单位
