...
/* 用转义字符定义部分控制字符 */
#define esc             '\033'        // 八进制表示,十六进制 '\x1b'
#define backspace       '\b'
#define tab             '\t'
#define enter           '\r'
#define delete          '\177'        // 八进制表示,十六进制 '\x7f'
...
static void intr_keyboard_handler(void) { 
    ...
    // 若扫描码 e0 开头,等待下一个扫描码拼接  
    if (scancode == 0xe0) {
        ext_scancode = true;    // 打开e0标记
        return;
    } 
    // 如果上次是以 0xe0 开头，合并扫描码 
    if (ext_scancode) {
    ...
    if (is_break_code) {    
        ...
        // CapsLock 不是弹起后关闭,单独处理 */
        return;   // 直接返回结束此次中断处理程序
    }
    // 通码,只处理数组中定义的键以及alt_right和ctrl_right键,全是make_code */
    else if ((scancode > 0x00 && scancode < 0x3b) || \
             (scancode == alt_r_make) || \
             (scancode == ctrl_r_make)) {
        bool shift = shift_down_last;  // 判断是否与shift组合,用来在一维数组中索引对应的字符
        // 我参照 macOS 的做法，对于字母，只要按了 shift 就一定是大写，不管 cap
        if (!shift && caps_lock_status && !( // 如果不是双符号的按键，即 ABC，
            (scancode <  0x0e) || (scancode == 0x29) || \
